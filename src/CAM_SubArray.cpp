	...
	if (CAM_cell->memCellType != SRAM) {
		...
		if (voltageSense) {						/* voltage-sensing */
			if (CAM_cell->readVoltage == 0) {  /* Current-in voltage sensing */
				...
				//cout << "voltageMemCellOn01: " << voltageMemCellOn*1e3 << endl;
				//cout << "res on: " << resMemCellOn*1e3 << ":" << CAM_cell->readCurrent * 1e3<< endl;
				...
				//cout << "voltageMemCellOn02: " << voltageMemCellOn*1e3 << endl;
				//voltagePrecharge = (voltageMemCellOff + voltageMemCellOn) / 2;
				//voltagePrecharge = MIN(tech->vdd, voltagePrecharge);  /* TO-DO: we can have charge bump to increase SA working point */
				if ((voltagePrecharge - voltageMemCellOn) <= senseVoltage) {
	...
	if (CAM_cell->accessType == CMOS_access || CAM_cell->accessType == diode_access){
		...
		if(numRow * tech->currentOffNmos[inputParameter->temperature - 300] / BITLINE_LEAKAGE_TOLERANCE >
			tech->currentOnNmos[inputParameter->temperature - 300]) {
				...
				cout << "numRow * tech->currentOffNmos[inputParameter->temperature - 300] / BITLINE_LEAKAGE_TOLERANCE: " << numRow * tech->currentOffNmos[inputParameter->temperature - 300] / BITLINE_LEAKAGE_TOLERANCE << endl;
				cout << "tech->currentOnNmos[inputParameter->temperature - 300]" << tech->currentOnNmos[inputParameter->temperature - 300] << endl;
			invalid = true;
		...
		if (typeSenseAmp == discharge) {
			if (CAM_cell->memCellType == SRAM || CAM_cell->accessType == CMOS_access) {
				...
				if (CAM_cell -> memCellType == FEFETRAM){
						...
						// 1/2-miss, 1/2-match situation
						// if (CAM_opt.BitSerialWidth == 1) {
						// 	resTotalCell = resMemCellOn;
						// } else {
						
						// resTotalCell = 2*(resMemCellOn + resMemCellOff)/CAM_opt.BitSerialWidth;
						// BaseResTotalCell = resMemCellOff/ArrayWidth;
			...
			if (CAM_cell->memCellType == SRAM) {
				...
				if (CAM_cell->accessType == none_access){
					...
					double resMisCell0, resMisCell1;
					double MLmargin = 40e-12;
					double tau0, tau1;
					double MLDelay0, MLDelay1;
					int MissBit;
					// cout << "AddCapOnML: " << inputParameter->AddCapOnML *1e15 << endl;
					// for (MissBit=0;MLmargin>=5e-12;MissBit++){
					// 	resMisCell0 = resMemCellOn*resMemCellOff/(resMemCellOn*(CAM_opt.BitSerialWidth-MissBit)+resMemCellOff*MissBit);
					// 	resMisCell1 = resMemCellOn*resMemCellOff/(resMemCellOn*(CAM_opt.BitSerialWidth-MissBit-1)+resMemCellOff*(MissBit+1));
					// 	tau0 = resMisCell0*(capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap + ColMux[indexMatchline].capForPreviousDelayCalculation + precharger.capOutputBitlinePrecharger + senseAmp.capLoad + inputParameter->AddCapOnML) 
					...
					// 	tau1 = resMisCell1*(capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap + ColMux[indexMatchline].capForPreviousDelayCalculation + precharger.capOutputBitlinePrecharger + senseAmp.capLoad + inputParameter->AddCapOnML) 
					// 		+ Col[indexMatchline].res * (ColMux[indexMatchline].capForPreviousDelayCalculation + capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap / 2);
					// 	MLDelay0 = horowitz(tau0, 0, RowDriver[indexMaxRowDriver].rampOutput, &bitlineRamp);
					// 	MLDelay1 = horowitz(tau1, 0, RowDriver[indexMaxRowDriver].rampOutput, &bitlineRamp);
					// 	MLmargin = MLDelay0 - MLDelay1;
					// 	cout << "Most detectable Mismatch Bit: " << MissBit << endl;
					// 	cout << "MLMargin: " << MLmargin *1e12 << endl;
					// }
					//cout << "Most detectable Mismatch Bit: " << MissBit << endl;
					...
					res0 = resMemCellOff/CAM_opt.BitSerialWidth;
					res1 = resMemCellOn*resMemCellOff/(resMemCellOn*(CAM_opt.BitSerialWidth-1)+resMemCellOff*1);
					res2 = resMemCellOn*resMemCellOff/(resMemCellOn*(CAM_opt.BitSerialWidth-2)+resMemCellOff*2);
					res3 = resMemCellOn*resMemCellOff/(resMemCellOn*(CAM_opt.BitSerialWidth-3)+resMemCellOff*3);
					res4 = resMemCellOn*resMemCellOff/(resMemCellOn*(CAM_opt.BitSerialWidth-4)+resMemCellOff*4);
					res5 = resMemCellOn*resMemCellOff/(resMemCellOn*(CAM_opt.BitSerialWidth-5)+resMemCellOff*5);
					BaseResTotalCell = resMemCellOff/CAM_opt.BitSerialWidth;
					
					
					double capTotalCell = 0;
					double tempbasetau = 0;
					tau0 = res0*(capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap + ColMux[indexMatchline].capForPreviousDelayCalculation + precharger.capOutputBitlinePrecharger + senseAmp.capLoad + inputParameter->AddCapOnML) 
							+ Col[indexMatchline].res * (ColMux[indexMatchline].capForPreviousDelayCalculation + capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap / 2);
					tau1 = res1*(capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap + ColMux[indexMatchline].capForPreviousDelayCalculation + precharger.capOutputBitlinePrecharger + senseAmp.capLoad + inputParameter->AddCapOnML) 
							+ Col[indexMatchline].res * (ColMux[indexMatchline].capForPreviousDelayCalculation + capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap / 2);
					tau2 = res2*(capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap + ColMux[indexMatchline].capForPreviousDelayCalculation + precharger.capOutputBitlinePrecharger + senseAmp.capLoad + inputParameter->AddCapOnML) 
							+ Col[indexMatchline].res * (ColMux[indexMatchline].capForPreviousDelayCalculation + capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap / 2);
					tau3 = res3*(capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap + ColMux[indexMatchline].capForPreviousDelayCalculation + precharger.capOutputBitlinePrecharger + senseAmp.capLoad + inputParameter->AddCapOnML) 
							+ Col[indexMatchline].res * (ColMux[indexMatchline].capForPreviousDelayCalculation + capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap / 2);
					tau4 = res4*(capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap + ColMux[indexMatchline].capForPreviousDelayCalculation + precharger.capOutputBitlinePrecharger + senseAmp.capLoad + inputParameter->AddCapOnML) 
							+ Col[indexMatchline].res * (ColMux[indexMatchline].capForPreviousDelayCalculation + capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap / 2);
					tau5 = res5*(capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap + ColMux[indexMatchline].capForPreviousDelayCalculation + precharger.capOutputBitlinePrecharger + senseAmp.capLoad + inputParameter->AddCapOnML) 
							+ Col[indexMatchline].res * (ColMux[indexMatchline].capForPreviousDelayCalculation + capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap / 2);
					
					//cout << "time0, time1, time2, time3, time4, time5 = " << tau0*log(2)*1e12 << ": " << tau1*log(2)*1e12 << ": " << tau2*log(2)*1e12 << ": " << tau3*log(2)*1e12 << ": " << tau4*log(2)*1e12 << ": " << tau5*log(2)*1e12 << endl;
					//cout << "0000tau" << tau * 1e12 << endl;
					//double capTotalCell = (Col[indexMatchline].cap + ColMux[indexMatchline].capForPreviousDelayCalculation)
					//		+ Col[indexMatchline].res * (ColMux[indexMatchline].capForPreviousDelayCalculation + Col[indexMatchline].cap / 2);
					...
					//SenseTime = log(2)*(tau-Basetau);
					//temptau = 1e12*(resTotalCell*(capCellAccess*CAM_opt.BitSerialWidth  + Col[indexMatchline].cap + ColMux[indexMatchline].capForPreviousDelayCalculation) 
					//		+ Col[indexMatchline].res * (ColMux[indexMatchline].capForPreviousDelayCalculation + capCellAccess*CAM_opt.BitSerialWidth  + Col[indexMatchline].cap / 2));
					//SenseTime = log(2)*(Basetau-tau);
					//double type = 1e12*(Col[indexMatchline].cap + ColMux[indexMatchline].capForPreviousDelayCalculation)
					//		+ Col[indexMatchline].res * (ColMux[indexMatchline].capForPreviousDelayCalculation + Col[indexMatchline].cap / 2);
					//MatchlineSenseMargin = log(2)*Basetau;
					//cout << "total cell cap" << (capCellAccess*CAM_opt.BitSerialWidth  + Col[indexMatchline].cap + ColMux[indexMatchline].capForPreviousDelayCalculation)*1e1  << endl;
					//cout << "ML res= " << Col[indexMatchline].res << endl;
					//cout << "base tau=" << Basetau*1e12*log(2) << endl;
					
					//MatchlineSenseMargin = voltagePrecharge* (exp(log(2)*tau/Basetau)- 0.5);
					//bitlineDelay = horowitz(tau, 0, RowDriver[indexMaxRowDriver].rampOutput, &bitlineRamp);
					//bitlineDelay = tau * log((voltagePrecharge - voltageMemCellOn) / (voltagePrecharge - voltageMemCellOn - senseVoltage));
						// TODO
					//bitlineDelayOff = tau * log((voltagePrecharge) / (voltageMemCellOn));
					// bitlineDelay = log(2)*tau + horowitz(tau, 0, RowDriver[indexMaxRowDriver].rampOutput, &bitlineRamp);
					// double bitlineDelay2 = log(2)*tau2 + horowitz(tau2, 0, RowDriver[indexMaxRowDriver].rampOutput, &bitlineRamp);
					// double bitlineDelay3 = log(2)*tau3 + horowitz(tau3, 0, RowDriver[indexMaxRowDriver].rampOutput, &bitlineRamp);
					// double bitlineDelay4 = log(2)*tau4 + horowitz(tau4, 0, RowDriver[indexMaxRowDriver].rampOutput, &bitlineRamp);
					// double bitlineDelay5 = log(2)*tau5 + horowitz(tau5, 0, RowDriver[indexMaxRowDriver].rampOutput, &bitlineRamp);

					bitlineDelay = horowitz(tau, 0, RowDriver[indexMaxRowDriver].rampOutput, &bitlineRamp);
					double bitlineDelay2 = horowitz(tau2, 0, RowDriver[indexMaxRowDriver].rampOutput, &bitlineRamp);
					double bitlineDelay3 = horowitz(tau3, 0, RowDriver[indexMaxRowDriver].rampOutput, &bitlineRamp);
					double bitlineDelay4 = horowitz(tau4, 0, RowDriver[indexMaxRowDriver].rampOutput, &bitlineRamp);
					double bitlineDelay5 = horowitz(tau5, 0, RowDriver[indexMaxRowDriver].rampOutput, &bitlineRamp);

					double search2 = inputBuf.readLatency + precharger.readLatency + maxRowDriver + inputEnc.readLatency + bitlineDelay2
					...
					double search3 = inputBuf.readLatency + precharger.readLatency + maxRowDriver + inputEnc.readLatency + bitlineDelay3
							+ ColMux[indexMatchline].readLatency + senseAmp.readLatency + outputAcc.readLatency + priorityEnc.readLatency
							+ outputBuf.readLatency;
					double search4 = inputBuf.readLatency + precharger.readLatency + maxRowDriver + inputEnc.readLatency + bitlineDelay4
							+ ColMux[indexMatchline].readLatency + senseAmp.readLatency + outputAcc.readLatency + priorityEnc.readLatency
							+ outputBuf.readLatency;
					double search5 = inputBuf.readLatency + precharger.readLatency + maxRowDriver + inputEnc.readLatency + bitlineDelay5
							+ ColMux[indexMatchline].readLatency + senseAmp.readLatency + outputAcc.readLatency + priorityEnc.readLatency
							+ outputBuf.readLatency;

					// cout << "time2, time3, time4, time5 = " << search2 *1e12 << ": " << search3*1e12  << ": " << search4*1e12  << ": " << search5*1e12 << endl;
					// cout << "bitdelay, bitdelay2, bitdelay3, bitdelay4, bitdelay5 = " << bitlineDelay * 1e12 <<": " << bitlineDelay2 *1e12 << ": " << bitlineDelay3*1e12  << ": " << bitlineDelay4*1e12  << ": " << bitlineDelay5*1e12 << endl;
					...
					// resM1 = S1/CAM_opt.BitSerialWidth;
					// resM2 = S2/CAM_opt.BitSerialWidth;
					// resM3 = S3/CAM_opt.BitSerialWidth;
					// resM4 = S4/CAM_opt.BitSerialWidth;
					// resM5 = S5/CAM_opt.BitSerialWidth;
					// resM6 = S6/CAM_opt.BitSerialWidth;
					// resM7 = S7/CAM_opt.BitSerialWidth;


					// Stau0 = resM0*(capCellAccess*CAM_opt.BitSerialWidth + inputParameter->AddCapOnML) 
					// 		+ Col[indexMatchline].res * (capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap / 2);
					// Stau1 = resM1*(capCellAccess*CAM_opt.BitSerialWidth + inputParameter->AddCapOnML) 
					// 		+ Col[indexMatchline].res * (capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap / 2);
					// Stau2 = resM2*(capCellAccess*CAM_opt.BitSerialWidth + inputParameter->AddCapOnML) 
					// 		+ Col[indexMatchline].res * (capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap / 2);
					// Stau3 = resM3*(capCellAccess*CAM_opt.BitSerialWidth + inputParameter->AddCapOnML) 
					// 		+ Col[indexMatchline].res * (capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap / 2);
					// Stau4 = resM4*(capCellAccess*CAM_opt.BitSerialWidth + inputParameter->AddCapOnML) 
					// 		+ Col[indexMatchline].res * (capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap / 2);
					// Stau5 = resM5*(capCellAccess*CAM_opt.BitSerialWidth + inputParameter->AddCapOnML) 
					// 		+ Col[indexMatchline].res * (capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap / 2);
					// Stau6 = resM6*(capCellAccess*CAM_opt.BitSerialWidth + inputParameter->AddCapOnML) 
					// 		+ Col[indexMatchline].res * (capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap / 2);
					// Stau7 = resM7*(capCellAccess*CAM_opt.BitSerialWidth + inputParameter->AddCapOnML) 
					// 		+ Col[indexMatchline].res * (capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap / 2);
					...
					// cout << "time2, time3, time4, time5 = " << search2 *1e12 << ": " << search3*1e12  << ": " << search4*1e12  << ": " << search5*1e12 << endl;
					// cout << "MCAM: bitdelay0, bitdelay2, bitdelay3, bitdelay4, bitdelay5,... = " << MbitlineDelay0 * 1e12 <<": " << MbitlineDelay1 *1e12 << ": " << MbitlineDelay2 *1e12  << ": " << MbitlineDelay3 *1e12  << ": " << MbitlineDelay4 *1e12 << ": " << MbitlineDelay5 *1e12  << ": " << MbitlineDelay6 *1e12 << ": " << MbitlineDelay7 *1e12 << endl;
					// cout << MbitlineDelay0 * 1e12 << endl;
					// cout << MbitlineDelay1 * 1e12 << endl;
					// cout << MbitlineDelay2 * 1e12 << endl;
					// cout << MbitlineDelay3 * 1e12 << endl;
					// cout << MbitlineDelay4 * 1e12 << endl;
					// cout << MbitlineDelay5 * 1e12 << endl;
					// cout << MbitlineDelay6 * 1e12 << endl;
					// cout << MbitlineDelay7 * 1e12 << endl;
				...
				//cout << "resOn" << resOn << endl;
				//cout << "tauM1" << tauM1 << endl;
				//cout << "gm" << gm << endl;

				// cout << "bitline delay" << bitlineDelay * 1e12 << endl;
				...
				if (CAM_cell->readMode == false) {	/* current-sensing */
					...
					/////////////////////////////////////////////////////////////////
					tau = Col[indexMatchline].res * (Col[indexMatchline].cap+ 0.6e-13* CAM_opt.BitSerialWidth)/
							2 * (resMemCellOff + Col[indexMatchline].res / 3) / (resMemCellOff + Col[indexMatchline].res);
					bitlineDelay = horowitz(tau, 0, RowDriver[indexMaxRowDriver].rampOutput, &bitlineRamp);
					cout << "bitline delay in Subarray 222: " << bitlineDelay * 1e12<< endl;
					...
					double res5 = resMemCellOn*resMemCellOff/(resMemCellOn*(CAM_opt.BitSerialWidth-5)+resMemCellOff*5);
						...
						cout << tau * 1e12 << "::" << voltagePrecharge << "::" << voltageMemCellOn << "::" << senseVoltage << endl;
						cout << "Bitline On: " << bitlineOn1 *1e12 << ": " << bitlineOn2 *1e12 << ": " << bitlineOn3 *1e12 << bitlineOn4 *1e12 << bitlineOn5 *1e12 << endl;
						cout << "Bitline Off: " << bitlineOff1 << ": " << bitlineOff2 *1e12 << ": " << bitlineOff3 *1e12 << bitlineOff4 *1e12 << bitlineOff5 *1e12 << endl;
						...
						cout << "bitline delay in Subarray 111: " << bitlineDelay * 1e12<< endl;
						cout << "tau: voltagePrecharge: voltageMemCellOn: voltageMemCellOff: sensevoltage " << tau *1e9 << ": " << voltagePrecharge *1e3 << ": " << voltageMemCellOn*1e3 << ": " << voltageMemCellOff *1e3<< ": "<< senseVoltage*1e3 << endl;
						cout << "voltageMemCellOn00: " << voltageMemCellOn*1e3 << endl;
						//////////////Approximate match////////////////////////////
						
						// Used for test ////////////////////////////
						// resTotalCell = resMemCellOn*resMemCellOff/(CAM_opt.BitSerialWidth-1)/(resMemCellOn+resMemCellOff/(CAM_opt.BitSerialWidth-1));
						// tau = resTotalCell*(capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap + ColMux[indexMatchline].capForPreviousDelayCalculation) 
						// 	+ Col[indexMatchline].res * (ColMux[indexMatchline].capForPreviousDelayCalculation + capCellAccess*CAM_opt.BitSerialWidth + Col[indexMatchline].cap / 2);
						// bitlineDelay = horowitz(tau, 0, RowDriver[indexMaxRowDriver].rampOutput, &bitlineRamp);
					} else {   /*Voltage-in voltage sensing */
						/////////////////////////////////test//////////////////////////
						...
						bitlineDelay = MAX(bitlineDelayOn, bitlineDelayOff);
						cout << "bitline delay in Subarray 000: " << bitlineDelay * 1e12<< endl;
					...
					double res5 = resMemCellOn*resMemCellOff/(resMemCellOn*(CAM_opt.BitSerialWidth-5)+resMemCellOff*5);
						...
						cout << "Bitline On: " << bitlineOn1 *1e12 << ": " << bitlineOn2 *1e12 << ": " << bitlineOn3 *1e12 << bitlineOn4 *1e12 << bitlineOn5 *1e12 << endl;
						cout << "Bitline Off: " << bitlineOff1 *1e12<< ": " << bitlineOff2 *1e12 << ": " << bitlineOff3 *1e12 << ": " << bitlineOff4 *1e12 << ": " << bitlineOff5 *1e12 << endl;
						...
						cout << "bitline delay in Subarray: " << bitlineDelay * 1e12<< endl;
				}
			}
		}
